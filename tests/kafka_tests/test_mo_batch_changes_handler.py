...
from kafka_config.batch_change_handler.handler import MOChangesHandler
...
async def test_changes_related_to_hierarchy_less_than_handler_min_default_value_successful_case_1(
    ...
    item = {"id": 1, "tmo_id": 1}
    msg_as_dict = {
        "objects": [item for x in range(MINIMUM_CHANGES_COUNT_TO_REBUILD - 1)]
    }

    handler = MOChangesHandler(session=session, message_as_dict=msg_as_dict)
...
async def test_if_changes_not_related_to_existing_hierarchies_successful(
    ...
    hierarchies"""
    not_existing_tmo_id = 255000

    stmt = select(Hierarchy).where(Hierarchy.id == not_existing_tmo_id)
    hierarchy_from_db = await session.execute(stmt)
    hierarchy_from_db = hierarchy_from_db.scalars().first()

    assert hierarchy_from_db is None

    item_not_releated_to_hierarhy = {"id": 255, "tmo_id": not_existing_tmo_id}
    msg_as_dict = {
...
async def test_changes_related_to_2_hierarchies_more_than_min_default_value_case_1(
    ...
    MINIMUM_CHANGES_COUNT_TO_REBUILD = 5
    item_1 = {"id": 1, "tmo_id": 1}
    items_for_hierarchy_1 = [
        item_1 for x in range(MINIMUM_CHANGES_COUNT_TO_REBUILD)
    ]

    item_2 = {"id": 1, "tmo_id": 3}
    items_for_hierarchy_2 = [
        item_2 for x in range(MINIMUM_CHANGES_COUNT_TO_REBUILD - 1)
    ]
