...
async def with_prm_delete_event(msg, session: AsyncSession, hierarchy_id: int):
...
async def with_prm_delete_event_parent_attr(
    ...
    for obj in response.scalars():  # type: Obj
        ...
        # find new parent obj
        parent_obj: Obj | None = None
        if obj.parent_id and level.parent_id:
            # parent level
            stmt = select(Level).where(Level.id == level.parent_id)
            response = await session.execute(stmt)
            parent_level: Level = response.scalar_one()

            # node
            if parent_level.object_type_id == level.object_type_id:
                stmt = select(NodeData.mo_id).where(NodeData.node_id == obj.id)
            else:
                stmt = select(NodeData.mo_p_id).where(
                    NodeData.node_id == obj.id
                )

            # parent node
            stmt = select(NodeData.node_id).where(
                NodeData.level_id == parent_level.id, NodeData.mo_id.in_(stmt)
            )

            # parent obj
            stmt = select(Obj).where(Obj.id.in_(stmt))

            response = await session.execute(stmt)
            parent_obj = response.scalar_one_or_none()
        ...
        obj.parent_id = parent_obj.id if parent_obj else None
        ...
        async for partition in result_generator.yield_per(
            ...
            for child_node in partition:
                child_node.path = child_node.path.replace(old_path, new_path)
...
class PRMDeleteHandler(HierarchyChangeInterface):
    async def make_changes(self):
        await with_prm_delete_event_parent_attr(
            msg=self.msg, session=self.session, hierarchy_id=self.hierarchy_id
        )
        await with_prm_delete_event(
            msg=self.msg, session=self.session, hierarchy_id=self.hierarchy_id
        )
